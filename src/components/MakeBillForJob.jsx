...
export default function MakeBillForJob({ open = true, onClose = () => {}, job }) {
...
const handleCloseClick = () => {
  ...
  message: "Please clear the bill item table before closing.",
  type: "warning",
});
...
const handleAmountChangeForPaymentMethod = (method, amount) => {
  ...
  const [jobRefInput, setJobRefInput] = useState(job)
  ...
  const [isBillSaved, setIsBillSaved] = useState(false);
    ...
    useEffect(() => {
    if (job?.jobRef) {
      setJobRefInput(job.jobRef); // auto-fill the input field when job prop changes
    } else {
      setJobRefInput(""); // reset when no job
    }
  }, [job]);
...
useEffect(() => {
  if (open) {
    // Reset bill maker, technician, and payments
    setBillMaker("");
    setSelectedTechnician("");
    setPayments([]);
    
    // Optional: reset items & jobRefInput if needed
    // setItems([]);
    // setJobRefInput("");
  }
}, [open]);
...
// useEffect(() => {
//     // Fetch bill number when modal opens
//     if (open) {
//       fetchBillNumber();
//     }
//   }, [open]);

  // Fetch bill number from backend
 ...
 if (field === "description" && containsDiscount(value)) {
    ...
    if (enteredPin !== discountPin) {
      alert(
        ...
        message: "Incorrect PIN. Discounts are not allowed.",
        type: "error",
      })  );
 ...
 function handleAddItem() {
  ...
  message: "Please enter a description before adding the item.",
  type: "warning",
});
    return;
  }
...
const handleAddStockItemToBill = (stockItem) => {
 ...
 async function handleSearchJob() {
    const input = jobRefInput.trim(); // what user typed (e.g. "IDSJBN-11-25-004")
    if (!input) {
      alert("Enter a job number");
      return;
    }

    const jobRef = input.toUpperCase(); // already has full IDSJBN-xxxx
 ...
 // 1️⃣ Populate customer fields
      // Populate customer fields with job data
      setCustomer({
        name: jobData.customerPrefix + ". " + jobData.customerName || "",
        contact: jobData.customerPhone || "",
        email: jobData.customerEmail || "",
        address: jobData.customerAddress || "",
        company: jobData.customerCompany || "",
      });

          // 2️⃣ Auto-fill bill items table with deviceType | model | faults
    const faultsText = Array.isArray(jobData.faults) ? jobData.faults.join(", ") : "";
    const deviceLine = `${jobData.jobRef || "-"} | ${jobData.deviceType || "-"} | ${jobData.model || "-"}${faultsText ? ` | ${faultsText}` : ""}`;

    setItems([
      {
        id: Date.now(),         // unique id for React key
        description: deviceLine, 
        finalLabel: deviceLine, 
        qty: 1,
        unitPrice: 0,           // leave 0, user can update
        amount: 0,
        isJobIssue: true,
      },
      // optionally keep existing items if needed: ...items
    ]);

      // alert(`Customer data for ${jobRef} loaded!`);
      showAlert({
  title: "Job Loaded",
  message: `Customer data for ${jobRef} loaded successfully.`,
  type: "success",
});
setJobRefInput(jobData.jobRef || "");
setIsJobLoaded(true);
    } catch (err) {
...
const handleDeleteItem = async (item) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error("Stock item not found");
      showAlert({
        title: "Stock Updated",
        message: `Stock quantity for ${item.finalLabel} was restored.`,
        type: "info",
      });
...
const UNDER_COST_PIN = "1234"; // 🔐 change to your actual secret PIN
 ...
 const handleSaveBill = async () => {
  ...
  message: "Customer details and at least one bill item are required.",
  type: "error",
})
  ...
  message: "You must select a technician before saving the bill.",
  type: "error",
}) );}

if (!billMaker) {
  return showAlert({
    title: "Bill Maker Required",
    message: "You must assign a bill maker before saving the bill.",
    type: "error",
  });
}

  // ✅ NEW VALIDATION: Ensure at least one payment exists
  if (!payments || payments.length === 0) {
    return showAlert({
      title: "Payment Required",
      message: "You must add at least one payment method before saving the bill.",
      type: "error",
    });
  }
  ...
  // ✅ NEW VALIDATION: Under-cost bill (negative profit)
if (Number(billProfit) < 0) {
  const enteredPin = prompt(
    "⚠️ You are saving an under-cost bill.\nPlease enter the secret PIN to continue:"
  );

  if (!enteredPin) {
    return alert("Under-cost bill cancelled. PIN is required.");
  }

  if (enteredPin !== UNDER_COST_PIN) {
    return alert("❌ Incorrect PIN. Under-cost bill was not saved.");
  }

  // ✅ Correct PIN → continue saving
}
  ...
  const billData = {
    ...
    jobRef: jobRefInput ? `${jobRefInput.toUpperCase()}` : "",
    customer,
    items,
    subTotal,
    billProfit,
  ...
  try {
    ...
    if (res.status === 201) {
      (
        showAlert({
          title: "Bill Saved",
          message: `Bill ${res.data.billNumber} saved successfully.`,
          type: "success",
        })
      );
      setIsBillSaved(true);
        ...
        jobCloseByBill(jobRefInput ? jobRefInput.toUpperCase() : "", res.data.billNumber);
    }
    return true;
  } catch (error) {
...
const jobCloseByBill = async (jobRef, billNumber) => {
  if (!jobRef || !billNumber) {
    return alert("Job reference and bill number are required to close the job");
  }

  const updatedFields = {
    jobProgress: `Closed By Bill - ${billNumber}`, // e.g., "Closed By Bill - IDSJBN-11-25-001"
  };
  ...
  try {
    ...
    if (res.status === 200) {
      (
        showAlert({
        title: "Job Closed",
        message: `Job ${jobRef} was closed by bill ${billNumber}.`,
        type: "success",
      }) );
    }
  } catch (error) {
...
//================ handel advanced payment back to previous status ===========================
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="grid grid-cols-1 gap-2 mt-2">
                 ...
                 <button   onClick={() => setShowViewPayments(true)}  className="px-3 py-2 bg-green-600 text-white font-bold rounded flex items-center justify-center gap-1 hover:bg-green-700 cursor-pointer"  > <span className="text-xs text-center">View Advanced Payments</span></button>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2 mt-2">
                  {/* <input className="border rounded px-3 py-2 text-xs font-bold" placeholder="Reference / IDSJBN Number" /> */}
                  <div className="flex gap-2 items-center mb-3">
                      <input
          type="text"
          className="border rounded px-3 py-2 text-xs flex-1"
          placeholder="Enter Job Number"
          value={jobRefInput}
          onChange={(e) => setJobRefInput(e.target.value)}
           readOnly={isJobLoaded}
        />
...
{/* <div className="mt-4">
  <p className="text-xs font-bold">Selected Payments:</p>
  <pre className="text-xs bg-gray-100 p-2 rounded">{JSON.stringify(payments, null, 2)}</pre>
</div> */}
      ...
      <td className="py-1 px-3 border text-xs w-[5%]">
        {it.isJobIssue ? (
              <button
                ...
                className="px-2 py-1 bg-green-500 text-white rounded text-xs cursor-pointer hover:bg-green-600"
              >
                Delete
              </button>
...
>
  <div
    ...
    style={{
          ...
          background: "#fff",
          fontFamily: "Inter, Arial, sans-serif",   // SHARP FONT
          fontSize: "12px",
          color: "#000", 
    }}
  >
...
{showStockModal && (
  <StockForBill
    ...
    onAddToBill={(stockItem) => {
      let newItem = {
      id: stockItem._id,          // ✅ Use real MongoDB _id
      isStockItem: true,          // ✅ Flag to differentiate stock items
      qty: 1,
      unitPrice: Number(stockItem.unitPrice) || 0,
      amount: Number(stockItem.unitPrice) || 0,
    }; 
      let finalLabel = ""; // will hold combined text
...
<SideAlert
  show={sideAlert.show}
  onClose={() => setSideAlert((prev) => ({ ...prev, show: false }))}
  title={sideAlert.title}
  message={sideAlert.message}
  type={sideAlert.type}
  autoClose={sideAlert.autoClose}
/>
 {showViewPayments && (
