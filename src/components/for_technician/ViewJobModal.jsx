...
const ViewJobModal = ({ job, onClose }) => {
  ...
  const renderField = (label, value) => (
    <div className="p-2 bg-gray-50 rounded shadow-sm">
      <p className="text-xs font-medium text-gray-600">{label}</p>
      <p className="text-sm font-semibold text-gray-800">{value ?? '-'}</p>
    </div>
  );

  // Updated: automatically parses stringified arrays and renders ordered lists
  const renderFullWidthField = (label, value) => {
    ...
    return (
      <div className="mb-3 p-2 bg-gray-50 rounded shadow-sm">
        <p className="text-xs font-medium text-gray-600">{label}</p>
        {Array.isArray(items) && items.length > 0 ? (
          <ol className="text-sm text-gray-800 list-decimal ml-5">
            {items.map((item, index) => (
              <li key={index}>{item}</li>
            ))}
          </ol>
        ) : (
          <p className="text-sm text-gray-800 whitespace-pre-wrap">{items ?? '-'}</p>
        )}
      </div>
    );
  };
    ...
    <Modal open={!!job} onClose={onClose}>
      <div className={modalStyle}>
        <div className={boxStyle}>
          ...
          <Typography variant="h5" className="text-green-600 font-bold mb-6 text-center">
            View Job Details
          </Typography><br /><br />
          ...
          <Accordion>
            <AccordionSummary expandIcon={<ArrowDropDownIcon />}>
              <Typography className="font-semibold text-gray-700">Customer Information</Typography>
            </AccordionSummary>
            <AccordionDetails>
              {renderGridSection([
          ...
          <Accordion>
            ...
            <AccordionDetails>
              {renderGridSection([
                ...
                { label: 'SIM Tray Collected', value: job.simTrayCollected ? 'Yes' : 'No' },
              ])}
            </AccordionDetails>
          </Accordion>
          ...
          <Accordion>
            ...
            <AccordionDetails>
              {renderGridSection([
                ...
                { label: 'Estimated Completion', value: job.estimatedCompletion },
          ...
          <Accordion>
            ...
            <AccordionDetails>
              {renderFullWidthField('Customer Reported Faults', job.customer_reported)}
              {renderFullWidthField('Collected Accessories', job.collected_accessories)}
              {renderFullWidthField('Tech Notes', job.repaired_accessories)}
              {renderFullWidthField('Faults', job.faults)}
