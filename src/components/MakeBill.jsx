...
async function handleDownloadPDF() {
  try {
    ...
    // ================= LOGO =================
    if (companyLogo) {
      pdf.addImage(companyLogo, "PNG", margin, y, 36, 18);
    }
    ...
    pdf.setFontSize(10);
    pdf.text(billNumber, pageWidth - margin, y + 12, { align: "right" });
    ...
    if (customer.company) {
      y += 4;
      pdf.text(customer.company, margin, y);
    }
    ...
    if (customer.address) {
      y += 4;
      pdf.text(customer.address, margin, y);
    }
    ...
    pdf.setFont("helvetica", "bold");
    pdf.setFontSize(9);
    pdf.text("Payments", pageWidth - margin - 45, payY);
    ...
    const descX = margin;
    const descW = pageWidth - margin - margin - 55;
    ...
    pdf.text("Description", descX + padding, tableTop + 4);
    ...
    pdf.text(
      `Total: Rs. ${Number(subTotal).toLocaleString()}`,
      pageWidth - margin,
      y,
      { align: "right" }
    );
    ...
    y += 3.5;
    pdf.text(
      "3. Issued items are considered sold and used.",
      margin,
      y
    );
...
const UNDER_COST_PIN = "1234"; // 🔐 change to your actual secret PIN
 ...
 const handleSaveBill = async () => {
  ...
  return showError(
    "Prefix Required",
    "Please select Mr / Mrs / Miss / Dr / Ven"
  );
}

// Combine prefix + name for validation & saving
...
const formattedName = normalizeName(customer.name);
const fullCustomerName = `${customer.prefix} ${formattedName}`.trim();


const fullNamePattern =
  /^(Mr\.|Mrs\.|Miss\.|Dr\.|Ven\.)\s[A-Z][a-z]+(\s[A-Z][a-z]+){1,2}$/;

if (!fullNamePattern.test(fullCustomerName)) {
  return showError(
    "Invalid Customer Name",
    "Select a prefix and enter First Last or First Middle Last."
  );
}
// ✅ Phone validation (skip if foreign number)
if (!isForeignNumber) {
  ...
  // Optional: minimal validation for foreign numbers
  if (!customer.contact || customer.contact.trim().length < 5) {
    return showError(
      "Invalid Phone Number",
      "Please enter a valid foreign phone number."
    );
  }
}
...
// ✅ NEW VALIDATION: At least one payment method required
if (!payments || payments.length === 0) {
  return showError(
    "No Payment Method Selected",
    "Please select at least one payment method before saving the bill."
  );
}
...
if (Number(billProfit) < 0) {
  ...
  const billData = {
    ...
    jobRef: jobRefInput ? `IDSJBN-${jobRefInput.toUpperCase()}` : "",
      customer: {
    ...customer,
    name: fullCustomerName, // ✅ SAVE COMBINED NAME
  },
    items,
    subTotal,
    billProfit,
...
const handleSearchCustomerByPhone = async () => {
  ...
  try {
    ...
    const normalizedPrefix = PREFIX_MAP[cust.prefix?.toUpperCase()] || "";
...
const handleSaveAndDownload = async () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-end p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full md:w-11/12 lg:w-4/5 max-h-[90vh] overflow-auto p-6 m-4">
        <style>{`
              ...
              <div>
               ...
               <div className="flex-2 ">
                 <select
                  ...
                  onChange={(e) =>
                    setCustomer({ ...customer, prefix: e.target.value })
                  }
                >
                  ...
                  {/* <input className="border rounded px-3 py-2 text-xs font-bold " placeholder="Customer Name" value={customer.name} onChange={(e) => setCustomer({ ...customer, name: e.target.value })} /> */}
                    <input
                      className="border rounded px-3 py-2 text-xs font-bold ml-4 w-4/5"
                      placeholder="First [Second] Last OR KN Rupasinghe (Do not type '.' in the name)"
                      value={customer.name}
                      onChange={(e) =>
                  ...
                  <div className="flex items-center gap-2">
                      {/* Foreign Number Checkbox */}
                <label className="flex items-center gap-1 text-xs font-semibold cursor-pointer">
                  <input
                    type="checkbox"
                    className="cursor-pointer scale-125 mr-2"
                      checked={isForeignNumber}
                     onChange={(e) => setIsForeignNumber(e.target.checked)}
                  />
                  Foreign Number
                </label>
                 <input
                    className="border rounded px-3 py-2 text-xs font-bold w-4/5"
                    placeholder={
                      isForeignNumber
                        ? "Enter foreign phone number"
                        : "01XXXXXXXX or 07XXXXXXXX"
                    }
                    value={customer.contact}
                    inputMode={isForeignNumber ? "text" : "numeric"}
                    maxLength={isForeignNumber ? undefined : 10}
                    onChange={(e) => {
                      const value = isForeignNumber
                        ? e.target.value
                        : e.target.value.replace(/\D/g, "");

                      if (!isForeignNumber && value.length > 10) return;
...
>
  ...
  <div style={{ flex: 1 }}>
    ...
    <div>{[customer.prefix, customer.name].filter(Boolean).join(" ")}</div>
